...
RESULTS_DIR="${SCRIPT_DIR}/results/centralized_$(date +%Y%m%d_%H%M%S)"
mkdir -p "${RESULTS_DIR}"
...
AGENT_LAUNCH_TIME=$((NUM_AGENTS / 2))  # 0.5s per agent
WARMUP_SECS=${WARMUP_SECS:-$((AGENT_LAUNCH_TIME + 20))}  # Add 20s for mesh formation
